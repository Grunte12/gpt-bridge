...
from chatgpt_api.api.admin import (
    account_capture_usage_summary as _account_capture_usage_summary,
    add_admin_check as _add_admin_check,
    admin_account_delete_payload as _admin_account_delete_payload_core,
    admin_accounts_response as _admin_accounts_response,
    admin_artifact_delete_payload as _admin_artifact_delete_payload,
    admin_artifacts_response as _admin_artifacts_response,
    admin_check as _admin_check,
    admin_db_path as _admin_db_path,
    admin_opencode_status as _admin_opencode_status,
    admin_store as _admin_store,
    console_command_for_config as _console_command_for_config,
    console_served_locally as _console_served_locally,
    console_url_for_config as _console_url_for_config,
    inspect_account_capture as _inspect_account_capture,
    inspect_account_capture_payload as _inspect_account_capture_payload,
    known_admin_account_names as _known_admin_account_names,
    opencode_backup_path as _opencode_backup_path,
    opencode_config_path as _opencode_config_path,
    opencode_config_with_chatgpt as _opencode_config_with_chatgpt,
    opencode_eject_payload as _opencode_eject_payload,
    opencode_inject_payload as _opencode_inject_payload,
    opencode_state_path as _opencode_state_path,
    project_root as _project_root,
    read_json_file as _read_json_file,
    redacted_headers as _redacted_headers,
    remove_empty_account_dir as _remove_empty_account_dir,
    safe_account_name as _safe_account_name,
    save_account_capture_payload as _save_account_capture_payload,
    send_admin_asset as _send_admin_asset,
    send_admin_root_redirect as _send_admin_root_redirect,
    send_console_redirect as _send_console_redirect,
    settings_from_admin_body as _settings_from_admin_body,
    unlink_expected_account_file as _unlink_expected_account_file,
    write_json_file as _write_json_file,
)
from chatgpt_api.api.artifacts import (
    DOWNLOAD_FILES as _DOWNLOAD_FILES,
    DOWNLOAD_FILES_LOCK as _DOWNLOAD_FILES_LOCK,
    download_file_id_from_path as _download_file_id_from_path,
    register_download_file as _register_download_file,
    send_download_file as _send_download_file,
)
...
from chatgpt_api.api.operations import (
    CHATGPT_OPERATIONS as _CHATGPT_OPERATIONS,
    CHATGPT_OPERATIONS_LOCK as _CHATGPT_OPERATIONS_LOCK,
    ChatGPTOperation as _ChatGPTOperation,
    cancel_chatgpt_operation as _cancel_chatgpt_operation,
    chatgpt_operation_cancel_requested as _chatgpt_operation_cancel_requested,
    create_chatgpt_operation as _create_chatgpt_operation,
    finish_chatgpt_operation as _finish_chatgpt_operation,
    get_chatgpt_operation as _get_chatgpt_operation,
    stop_chatgpt_operation_by_id as _stop_chatgpt_operation_by_id,
    update_chatgpt_operation as _update_chatgpt_operation,
)
...
def _account_supports_model(
    ...
    if model_slug == "gpt-5-6-thinking":
        return thinking_effort in set(capabilities.get("sol_efforts") or [])
    return True


def run_server(config: OpenAICompatConfig, *, on_ready: Callable[[], None] | None = None) -> None:
    ...
    print(f"GPT Bridge server listening on http://{config.host}:{config.port}")
...
def _raw_query_value(query: dict[str, list[str]], key: str) -> str:
    """Read a query value without lower-casing opaque capability tokens."""
...
def _handler_class(config: OpenAICompatConfig, router: AccountRouter | None = None):
    ...
    class OpenAICompatHandler(BaseHTTPRequestHandler):
        server_version = "gpt-bridge-openai-compat/0.1"
        ...
        def do_HEAD(self) -> None:  # noqa: N802
            ...
            if download_id:
                _send_download_file(
                    ...
                    include_body=False,
                    access_token=_raw_query_value(parse_qs(parsed_url.query), "access_token"),
                )
                return
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if download_id:
                _send_download_file(
                    ...
                    download_id,
                    admin_db_path=config.admin_db_path,
                    access_token=_raw_query_value(query, "access_token"),
                )
                return
            if path == "/admin" or path == "/admin/" or path.startswith("/admin/"):
                if _console_served_locally(config):
                    if path == "/admin":
                        _send_admin_root_redirect(self)
                    else:
                        _send_admin_asset(self, path)
...
async def _chat_completion(
    ...
    use_agent_bridge = _should_use_agent_bridge(body, tools, model_agent_mode)
    if not use_agent_bridge:
        messages = _ordinary_chat_messages(config, messages, body)
        ...
        try:
            try:
                ...
                if not fallback_model or not _should_try_fallback_model(exc):
                    raise
    ...
    try:
        ...
        if not use_agent_bridge:
            ...
            def send_delta(delta: str) -> None:
                ...
                _write_sse_content(handler, chunk_base, delta)
            ...
            try:
                ...
                fallback_model = _model_fallback_for_config(config, model_slug)
                if emitted_content or not fallback_model or not _should_try_fallback_model(exc):
                    raise
                fallback_model_slug, fallback_effort = _resolve_model_alias(fallback_model, None)
                chunk_base["chatgpt_fallback_model"] = fallback_model_slug
                await _stream_messages_text_with_accounts(
                    config,
                    router,
                    messages,
    ...
    for account, preflight_metadata in await _account_order_with_usage_preflight(
        ...
        if input_image_count:
            ...
            if preflight_error is not None:
                ...
                _record_account_attempt(attempts, account, compat_error)
...
def _admin_account_delete_payload(config: OpenAICompatConfig, body: dict[str, Any]) -> dict[str, Any]:
    router = AccountRouter(_accounts_for_config(config), config.account_strategy)
    return _admin_account_delete_payload_core(config, body, router)
...
def _ordinary_chat_messages(
    config: OpenAICompatConfig,
    messages: list[Any],
    body: dict[str, Any] | None = None,
) -> list[Any]:
    """Add the configured plain-chat policy without mutating caller input."""
    preset_name = _preset_name_from_body(body)
    for message in messages:
        if not isinstance(message, dict):
            continue
        role = (_str_or_none(message.get("role")) or "").strip().lower()
        if role == "system":
            return messages
    if preset_name:
        return [{"role": "system", "content": PRESETS[preset_name]}, *messages]
    if config.concise_system_prompt:
        return [{"role": "system", "content": DEFAULT_CONCISE_SYSTEM_PROMPT}, *messages]
    return messages


def _preset_name_from_body(body: dict[str, Any] | None) -> str | None:
    metadata = body.get("metadata") if isinstance(body, dict) and isinstance(body.get("metadata"), dict) else {}
    value = metadata.get("chatgpt_preset")
    if value is None:
        return None
    if not isinstance(value, str) or not value.strip():
        raise ValueError("chatgpt_preset must be one of: " + ", ".join(sorted(PRESETS)))
    name = value.strip()
    if name not in PRESETS:
        raise ValueError("unknown chatgpt_preset: " + name)
    return name
...
def _record_account_attempt(
    attempts: list[dict[str, Any]], account: str, exc: OpenAICompatProviderError
) -> None:
    """Append an attempt and expose the complete history on terminal errors."""
    attempts.append(_account_attempt_summary(account, exc))
    exc.account_attempts = attempts.copy()
    ...
    *,
    ...
    if not capture_path.exists():
        raise ProviderError(
            ...
            "`gpt-bridge admin account add --paste --base-url http://127.0.0.1:8000/v1 --api-key YOUR_LOCAL_KEY`."
        )
...
def _models_for_account(config: OpenAICompatConfig, account: str | None = None) -> list[dict[str, str]]:
    ...
    if capabilities.get("sol_model") and "extended" in capabilities.get("sol_efforts", []):
        models.append({"id": "gpt-5-6-sol-high", "name": "GPT-5.6 Sol High"})
    return _models_with_agent_modes(models)
